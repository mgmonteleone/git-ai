...
- **Augment Code (Auggie CLI) preset and hook installer**: First-class support for the Augment Code CLI — both Auggie v1 and v2 (cosmos-agent) — autodetected per-payload from its shape, one preset, no config needed ([#1071](https://github.com/git-ai-project/git-ai/issues/1071), CSS-2302)
  ...
  - v2 support: `write`/`edit`/`bash` tool attribution, workspace root recovered from the hook process's inherited cwd, and best-effort, bounded session id + model mining from the newest on-disk session file, with graceful fallback to a stable hash and `"unknown"` model on any failure
